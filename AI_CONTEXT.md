...
- **Minimum net-edge filter (v1)** (`modules/swap_executor.py`): **USDC→equity** (X-Signal) and **USDT→WMATIC** (main strategy) entries must show estimated **net** edge ≥ **`MIN_NET_EDGE_PCT`** (env, default **1.8%**; module fallback `_MIN_NET_EDGE_PCT`) after a rough gas estimate (`expected gross − gas`, as % of notional). Applied **early** in `determine_trade_decision` using planning gas **`NET_EDGE_PLANNING_GAS_GWEI`** (default **80** gwei, no RPC) and again at swap execution in `main()` with live `get_gas_status()` gas. Exits and copy paths are unchanged. Log: `[nanoclaw] Low edge rejected | expected_net=…% | reason=below_min_net_edge (floor=…%)`. First quality gate toward learning from low-edge mistakes.
...
- **P2 Profit-Take Relief** (TEMPORARY SPRINT FIX — May 2026; revert after sprint window):
  ...
  - **Stable rotation fallback (Signal-Driven Rotation — May 2026)**: after **6+** cycles without a WMATIC→stable profit exit and combined stables **≥ $50**, `determine_trade_decision` may return an executable **USDC→EQUITY** X-SIGNAL plan with `signal_strength ≥ 0.75` before default USDT→WMATIC accumulation (`MAIN_STRATEGY_STABLE_ROTATION_FALLBACK`). Logs: `[nanoclaw] Signal-Driven Rotation: stable fallback | …` and `🔍 DECISION PATH: MAIN_STRATEGY_STABLE_ROTATION_FALLBACK`.
  - **Main-strategy observability**: each cycle logs `[nanoclaw] MAIN_STRATEGY_STATUS` (wm USD, low-stack flag, cycles since exit, stables, profit-take reason, active P2/force thresholds, stable-fallback readiness) and `[nanoclaw] MAIN_STRATEGY_OUTCOME` (direction, actionable, quiet, note) so ops can see why the branch traded or went quiet. WMATIC band exits also emit `DECISION | branch=MAIN_STRATEGY` with reasons `wmatic_high_take_profit`, `usdt_reserve_protection`, `wmatic_low_cut_loss`, `rotation_priority_defer_buy`, etc. Open-trade TP uses `DECISION | branch=PROFIT_TAKE` from `evaluate_take_profit` (`TP_HIT`, `TRAILING_STOP_HIT`, `HOLD`, …).
