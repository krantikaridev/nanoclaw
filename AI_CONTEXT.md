...
- **P2 Profit-Take Relief** (TEMPORARY SPRINT FIX — May 2026; revert after sprint window):
  ...
  - `_MAIN_STRATEGY_PROFIT_TAKE_BALANCE_RELIEF_NOTIONAL_FLOOR_USD = 5.0` — P2 relief notional floor (May 2026; raised from $2.0) to block sub-$5 micro exits that burn gas on healthy stacks.
  ...
  - **Tiered P2/force (Signal-Driven Rotation — May 2026)**: reduces over-reliance on high WMATIC balance via **`stack_tier`** (`low` / `moderate` / `healthy` in `modules/swap_executor.py`):
    - **Low** (stack **< $7**): P2 wm **$1.75**, signal **0.45**, force wm **$1.75**, force after **1** idle cycle ($5–$8 band uses same fast path).
    - **Moderate** (**$7–$15**): P2 wm **$7**, signal **0.50**, force wm **$7**, force after **4** cycles (between depleted and healthy; tightened May 2026 vs $4 / 3-cycle micro exits).
  ...
  - **Long-idle fallback (May 2026)**: after **3** / **6** / **8** cycles without a WMATIC→stable exit (low / moderate / healthy tier), **`long_idle_active`** lowers force notional floor to **$1.35** and force wm floor to **$1.50** so micro rotations can still run (gas-safe; not sub-dollar). Log: `[nanoclaw] MAIN_STRATEGY long idle fallback | …`.
  - **Mild-loss micro rotation (May 2026)**: HOLD with unrealized **-7% to -1%**, WMATIC **$5–$10**, **1+** idle cycle → capped sell (~30%, max **$2.25** notional, floor **$1.25**); bypasses global **`MIN_TRADE_USD`** via `_main_strategy_mild_loss_rotation_min_notional_bypass_allowed` (decision + execution). Logs: `[nanoclaw] MILD-LOSS RECOVERY BYPASS ACTIVE | path=… | WMATIC=… | notional=… | gain_pct=… | cycles_since_exit=… | bypassing min_notional=…`; `[Main Strategy] Mild-loss rotation executing (bypassed min_notional for recovery)`; `[Main Strategy] Small idle rotation triggered | … | reason=recover_mild_loss` (reversible sprint tuning). Mild-loss idle/fast recovery also bypasses X-Signal P2 defer (same as fast path).
  - **Mild-loss fast rotation (TEMPORARY May 2026)**: same band + **WMATIC qty > 10** + **USDT ≥ `MAIN_STRATEGY_MIN_USDT_RESERVE`** → **0** idle cycles (next cycle), max **$2.75** notional; log `[Main Strategy] Mild-loss fast rotation | … | reason=accelerate_recovery`. P2 relief still runs when Signal-Driven Rotation would defer other WMATIC→stable exits. Remove the `TEMPORARY` block in `modules/swap_executor.py` to revert.
