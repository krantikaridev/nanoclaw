...
- **Fix (Cleanup #6)**: When combined stables `< MAIN_STRATEGY_LOW_STABLES_DUST_REBUILD_MAX_STABLE_USD` ($15), portfolio `>$130`, `followed_equity_usd/total ≥ FE_STABLE_RUNWAY_MIN_FE_SHARE` (default 0.55), and WMATIC stack cannot fund rebuild (`wmatic_usd < $5` or WMATIC=0): plan partial **EQUITY→USDC** trim on largest FE holding toward `FE_STABLE_RUNWAY_TARGET_STABLE_USD` (default $40) **before** any USDC→EQUITY BUY; hard-block BUY even when `ALLOW_REDUCED_HIGH_RISK_XSIGNAL` would re-enable entries.
