...
- **P2 Profit-Take Relief** (TEMPORARY — 48-hour sprint May 2026; revert after sprint window):
  ...
  - `_MAIN_STRATEGY_PROFIT_TAKE_BALANCE_RELIEF_NOTIONAL_FLOOR_USD = 3.0` — gas guard for sub-$3.00 exits (lowered 20 May from $3.50: production profit-take sizes land at ~$3.38–$3.39 and were still blocked; prior 19 May drop from $5.0; minimal sprint tweak, reversible).
  - `_MAIN_STRATEGY_PROFIT_TAKE_BALANCE_RELIEF_MIN_SIGNAL_STRENGTH = 0.55` — bypass uses `_profit_take_balance_relief_signal_strength()`: prefers `profit_signal['signal_strength']`, else May 2026 sprint heuristics (TP_HIT / TRAILING_STOP_HIT / STRONG_TP_HIT keep a 0.58–0.65 floor on modest gains; positive `gain_pct` / `peak_gain_pct` boost; healthy WMATIC > $7 adds +0.03/+0.05 so weak signals are not ~0.00; never returns exactly 0.00 when there is a valid exit or positive gain), then `decision.signal_strength`. Strengths are rounded to two decimals.
  ...
  - **Precedence (May 2026 sprint):** P2 relief bypass is evaluated **before** `MAIN_STRATEGY` dust defer for `WMATIC_TO_USDT` / `WMATIC_TO_USDC` so small exits are not hard-blocked by `min_notional_usd` before relief can run — improves capital rotation from the main strategy path.
  - Logs: `[nanoclaw] P2 relief check | wm=$X | notional=$Y | signal=$Z | allowed=$bool` (includes `floor` and `reason` on fail, e.g. `notional_below_floor`); on pass also `[nanoclaw] Main strategy small profit take allowed (P2 relief)`; when bypass overrides MAIN_STRATEGY dust defer: `[nanoclaw] P2 relief override | Allowing small profit take below min_notional (notional=$X)`.
  - **Force small profit take** (TEMPORARY — May 2026 sprint): `state.profit_take_rotation.cycles_since_exit` increments each `determine_trade_decision` run; resets on successful `WMATIC_TO_USDT` / `WMATIC_TO_USDC`. After **8+** cycles without exit, healthy WMATIC (>$7) + notional **≥ $3.00** hard floor (and > $2.50 soft check) can force-allow weak-signal relief — log `[nanoclaw] FORCE small profit take | WMATIC healthy but no exit for X cycles`; P2 check reason `force_no_exit_cycles`.
