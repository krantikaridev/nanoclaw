...
- **P2 Profit-Take Relief** (TEMPORARY / SPRINT — May 2026; revert after sprint window):
  ...
  - `_MAIN_STRATEGY_PROFIT_TAKE_BALANCE_RELIEF_NOTIONAL_FLOOR_USD = 3.0` — gas guard for sub-$3.00 exits (production profit-take sizes land at ~$3.38–$3.39).
  - `_MAIN_STRATEGY_PROFIT_TAKE_BALANCE_RELIEF_MIN_SIGNAL_STRENGTH = 0.55` — bypass gate; `_profit_take_balance_relief_signal_strength()` prefers `profit_signal['signal_strength']`, else sprint heuristics. **Final May 2026 fix:** when WMATIC ≥ $7, strength never below **0.50**; non-HOLD exit reasons (TP_HIT, TRAILING_STOP_HIT, etc.) never below **0.60** on a healthy stack — stops ~0.00 scores from blocking rotation. Strengths rounded to two decimals.
  ...
  - **Precedence:** for `WMATIC_TO_USDT` / `WMATIC_TO_USDC`, `_profit_take_balance_relief_bypass_allowed()` runs **before** `MAIN_STRATEGY` dust defer / `$10` `min_notional_usd` so P2 relief can override the hard floor.
  - Logs: `[nanoclaw] P2 relief check | wm=… | notional=… | signal=… | allowed=…`; `[nanoclaw] Main strategy small profit take allowed (P2 relief)`; main-strategy override: `[nanoclaw] P2 relief overriding min_notional for main strategy exit (notional=$X)`.
  - **Force small profit take** (TEMPORARY / SPRINT): `state.profit_take_rotation.cycles_since_exit` increments each `determine_trade_decision`; resets on successful WMATIC→stable exit. After **6+** cycles without exit, WMATIC stack **≥ $8** and notional **≥ $3.00** can force-allow weak-signal relief — `[nanoclaw] FORCE small profit take | WMATIC healthy, no exit for X cycles`; P2 reason `force_no_exit_cycles`.
