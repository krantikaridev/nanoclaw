...
- **Ship package (`0ee1302a` + follow-ups)**:
  ...
  2. **`MAIN_STRATEGY_LOW_STABLES_DUST_REBUILD_*`**: When stables `<$15`, portfolio `>$130`, and MAIN wants a **$5–$8** WMATIC→stable exit, bypass dust defer at planning (rate-limited every 3 cycles) and **`MIN_TRADE_USD` at execution** via `pending_execution` + `min_trade_guard bypassed` log.
