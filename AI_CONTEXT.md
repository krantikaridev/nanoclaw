...
- **Track B — dev**: Branch **`V4-play`** on an **ephemeral VM** (`STAGE_SEED_USD≈50`, **~$50 wallet** — see **`docs/DEV_ENV.md`**). Not 24/7; destroy after test. Adds dual-window unpause (8h+12h), play budget (2 fills/day on books &lt; $200), negative-window X-Signal cap ($10). Safe swap table: **`docs/ROTATION_PLAYBOOK.md`**. Sprint: **`docs/AGENT_SPRINT_PROMPTS_V4.md`**. **TODO (infra):** one-command cloud-agnostic lab bootstrap — **`docs/INFRA_AUTOMATION_BACKLOG.md`** (Oracle 2× free + other clouds).
...
- **X-SIGNAL blocked symbols** (`modules/signal.py`): repo-root (or cwd / `followed_equities.json` dir) file **`.xsignal_blocked_symbols`** — one symbol per line (e.g. `WMATIC_ALPHA`, `WETH_ALPHA`). Loaded at cycle start (mtime-cached). Blocked symbols are dropped **before** eligibility, gas pre-flight, and `build_plan` in `try_x_signal_equity_decision` / `evaluate_x_signal_equity_trade`; execution paths re-check via `_x_signal_apply_blocked_symbol_filter` in `modules/swap_executor.py`. Missing file → no blocks (graceful). If the block list would remove **all** followed assets: with **`X_SIGNAL_HONOR_FULL_BLOCKLIST=true`** (recommended when intentionally blocking everything) → no eligible assets, log `[X-SIGNAL] All followed assets blocked — no equity plans (operator block honored)`; with **`false`** (default, backward compatible) → blocks **ignored for that cycle** with WARNING `ignoring blocks this cycle`. Loss-cut always honors full blocklist (`allow_ignore_all_blocked=False`). Optional env skip list: **`X_SIGNAL_TEMP_SKIP_SYMBOLS`** (default empty). Template: **`.xsignal_blocked_symbols.example`**. Log: `[X-SIGNAL] Skipping blocked symbol: SYM (from .xsignal_blocked_symbols)`.
...
- **POL auto top-up vs real swap cost (@aniki, post-v2.8.0)**: **`AUTO_TOPUP_POL`** / **`ensure_pol_for_trade`** target **`_pol_target_for_trade()`** = `max(operating_floor, POL_EXECUTION_GAS_UNITS × live_gwei × POL_EXECUTION_GAS_MULTIPLIER)` so POL above **`MIN_POL_FOR_GAS`** but below approve+swap cost still unwraps WMATIC (USDT leg optional). **`nanoup`** preserves **`ALLOW_HIGH_RISK_LOSS_CUT_XSIGNAL`** via **`ENV_APPLY_PRESERVE_KEYS`**. Loss-cut blocked at execution when flag false or dust notional.
...
Instance A had **8 Polygon token contracts** (USDC, USDT, WMATIC, …) in `followed_wallets.json` instead of trader EOAs — `copy_targets=8` but no polycopy edge; **X-SIGNAL equities** (`followed_equities.json`) drive rotation. Fix: **`docs/COPY_TRADING_AUDIT.md`** + **`nanocopyaudit`** (`scripts/copy_trading_audit.py`); `modules/copy_trading_audit.py` classifies known token contracts; repo list is **empty** until operator adds verified EOAs; runtime strips tokens when `COPY_TRADING_REJECT_TOKEN_CONTRACTS=true` (`get_target_wallets()`). **Stage safe default:** `COPY_TRADING_ENABLED=false` in `.env.example` until `nanocopyaudit` exit **0**; then add 1–2 Polygonscan EOAs at `max_copy_ratio` 0.05–0.08 and confirm `copy_targets=N` matches tradeable count only.

- **Leave (Instance A):** `paused+lock` + pause skip lines = **not trading entries**. **`pgrep clean_swap` empty is normal** — bot runs **one cycle per cron tick** then exits (`*/2 * * * *` watchdog in VM crontab). Liveness = **recent `real_cron.log` timestamps**, not persistent PID.
...
- **Fix (Cleanup #6)**: When portfolio `>$130` and `followed_equity_usd/total ≥ FE_STABLE_RUNWAY_MIN_FE_SHARE` (default 0.55): **hard-block** all USDC→EQUITY / X-SIGNAL BUY while combined stables `< FE_STABLE_RUNWAY_TARGET_STABLE_USD` (default $40), even when stables `≥ $15` and WMATIC can rebuild. **Trim** (partial EQUITY→USDC toward runway target) only when stables `< MAIN_STRATEGY_LOW_STABLES_DUST_REBUILD_MAX_STABLE_USD` ($15) and WMATIC rebuild path absent (`wmatic_usd < $5` or WMATIC=0). `nanoup` preserves **`X_SIGNAL_HONOR_FULL_BLOCKLIST`** via **`ENV_APPLY_PRESERVE_KEYS`**.
- **Logs**: `FE STABLE RUNWAY TRIM | sym=… | sell_fraction=… | stable_usd=… | fe_share=… | target_stable_usd=…`; `FE STABLE RUNWAY | defer USDC→EQUITY BUY | stable_usd=… | fe_share=…`.
- **WMATIC-era unchanged**: When `wmatic_usd ≥` rebuild floor, existing `MAIN_STRATEGY_LOW_STABLES_DUST_REBUILD_*` WMATIC→stable path still wins.
...
- **Fix (P1)**: `runtime._followed_equity_tokens_usdt_usd` persists **`last_good_spot_usd`** per symbol to **`.runtime/fe_usd_spot_cache.json`** when live quote **≥ `bal × effective_floor`** (degraded quotes that lose to the fallback floor do **not** overwrite cache). **`effective_floor_px = min(json_floor, prior_last_good)`** (first healthy live anchors when cache cold). Per-asset USD stays **`max(live, bal × effective_floor)`** (Cleanup #3 preserved). Upward cache drift capped by **`FE_USD_SPOT_CACHE_MAX_UP_PCT_PER_DAY`** (default **5%**). Logs: **`FE_USD AUTO_FLOOR_UPDATE`** (spot changed only), existing fallback/unquoted lines use **effective** floor px.
- **Acceptance**: With stale JSON **2500** and healthy live **~$79.53** on **0.039756 WETH**, **TOTAL** within **~$1** of MetaMask Polygon tab; no **`FE_USD FALLBACK FLOOR APPLIED`** when live ≥ `bal × effective_floor`.
- **Out of scope**: `signal_strength`, trading decisions, Cleanup #6.
...
- **Ship package (`0ee1302a` + follow-ups)**:
  ...
  2. **`MAIN_STRATEGY_LOW_STABLES_DUST_REBUILD_*`**: When stables `<$15`, portfolio `>$130`, and MAIN wants a **$5–$8** WMATIC→stable exit, bypass dust defer at planning; **`MIN_TRADE_USD` at execution** via `pending_execution` + `min_trade_guard bypassed`. Cooldown starts only after a successful swap; pending retries are not rate-limited. **Stable-first**: rotation-priority X-SIGNAL BUY defers when rebuild is urgent.
...
- **Post-deploy fix**: Live spot from on-chain quote; bootstrap entry uses `HIGH_RISK_LOSS_CUT_BOOTSTRAP_ENTRY_PREMIUM_PCT` (default 12%). `HIGH_RISK_LOSS_CUT_WHEN_UNDERWATER_ANY_RISK=true` trims when LINK underwater even if `Risk=LOW`. `HIGH_RISK_LOSS_CUT_SPOT_SANITY_MULT` (default 1.35) caps inflated pool quotes vs FE fallback. Loss-cut runs in **`determine_trade_decision`** after protection (no `ENABLE_X_SIGNAL_EQUITY` gate). Per-asset cooldown is persisted in **`bot_state.json`** (`asset_last_trade_unix`) so cron one-shots honor `PER_ASSET_COOLDOWN` / X-SIGNAL cooldown seconds; loss-cut also **pre-marks** cooldown before approve/swap. **`NANOCLOW_CYCLE_LOCK_SECONDS`** (default 300) prevents overlapping cron processes (old 15s lock expired mid-swap → four rapid trims). **`HIGH_RISK_LOSS_CUT_MIN_EQUITY_USD`** skips dust/zero LINK plans (STF/gas). Loss-cut honors **`.xsignal_blocked_symbols` even when LINK is the only followed asset** (`allow_ignore_all_blocked=False`; set **`X_SIGNAL_HONOR_FULL_BLOCKLIST=true`** on the normal equity path to match). Grep: `loss-cut evaluate`, `skipped=dust`, `loss-cut inactive`, `loss-cut deferred`.
...
- **Root cause #4 — WBTC balance-read spam**: `.xsignal_blocked_symbols` containing `WBTC_ALPHA` only gated trading, not inventory reads. The FE_USD scan still tried to read `WBTC_ALPHA` (0x1BFD6703…6C834E, `BadFunctionCallOutput`) every cycle, emitting `BALANCE READ FAILED` lines indefinitely.
- **Fix #4 (Cleanup #3 part 4)**: Added a per-`(token, wallet)` log-once latch (`_BALANCE_READ_FAIL_LOGGED`) in `get_token_balance`. **May 2026 follow-up**: FE_USD inventory skips **`balanceOf` only for known-broken contracts** (Polygon WBTC `0x1BFD6703…`) — **not** for `.xsignal_blocked_symbols` entries, so blocking WETH for trading cannot collapse `TOTAL` to stables-only. Tests in `tests/unit/test_runtime_get_token_balance.py`.
