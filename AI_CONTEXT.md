...
- **P2 Profit-Take Relief** (TEMPORARY SPRINT FIX — May 2026; revert after sprint window):
  ...
  - **Force small profit take** (TEMPORARY SPRINT FIX — capital rotation): `state.profit_take_rotation.cycles_since_exit` increments each `determine_trade_decision`; resets on successful WMATIC→stable exit. After **4+** cycles without a WMATIC→stable profit take, stack **≥ $6.5** and notional **≥ $1.80** (below `MIN_TRADE_USD`) force-allow P2 relief — bypasses standard **$7** / **$2.00** / **0.55** gates and **`min_notional_usd`** like the override path. Log: `[nanoclaw] FORCE small profit take | WMATIC healthy, no exit for N cycles | notional=$X.XX`; P2 reason `force_no_exit_cycles`.
