...
- **P2 Profit-Take Relief** (TEMPORARY SPRINT FIX — May 2026; revert after sprint window):
  ...
  - **Anti-churn accumulate guards (May 2026)**: `MAIN_STRATEGY_ACCUMULATE_COOLDOWN_CYCLES` (default **6**) blocks `USDT→WMATIC` for N cycles after each successful WMATIC→stable exit (`profit_take_rotation.cycles_since_exit`). `MAIN_STRATEGY_ACCUMULATE_MAX_WMATIC_USD` (default **22**) defers buys when WMATIC stack is already at target. `control.json` **`paused=True`** takes precedence over accumulate defer. Env-tunable in `config.py`.
