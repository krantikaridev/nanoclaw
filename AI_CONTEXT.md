...
| **`FE_USD`** | **USDT-notional** for **non-core** rows in **`followed_equities.json`** (e.g. **Polygon WETH** `0x7ceB…`)—same `balanceOf` on **Polygon** as stables. **Not** Ethereum-mainnet WETH (`0xC02a…`); MetaMask totals often mix chains. Uses **V2 router**, **QuoterV2**, multihop; optional **`current_price_usd × balance`**. **`FE_USD=$0`** with “WETH” in MetaMask: confirm you’re on **Polygon** token details (contract **`0x7ceB…`**) or expect **zero** on-chain for that asset on Polygon; look for **`BALANCE READ FAILED`** if reads error. **Silent zero-quote diagnostic (May 2026)**: when `bal>0` but every quote tier returns 0, `runtime._followed_equity_tokens_usdt_usd` now emits `[nanoclaw] FE_USD UNQUOTED \| sym=… \| bal=… \| fallback_px_usd=… \| contributed_to_total=$…` to `real_cron.log`. `current_price_usd` in `followed_equities.json` is the **fallback floor**; populated for `WETH_ALPHA` / `WBTC_ALPHA` / `LINK_ALPHA` so a single broken on-chain quote path no longer silently undercounts `TOTAL`. Live quotes always take precedence — fallback is only used when all 4 tiers return 0. Refresh fallbacks periodically; they're only a safety net. |
