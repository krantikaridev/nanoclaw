...
| **`FE_USD`** | **USDT-notional** for **non-core** rows in **`followed_equities.json`** (e.g. **WETH**—same `balanceOf` path as USDT, not a separate oracle). Uses **V2 router**, **QuoterV2** single-hop, legacy **Quoter**, then **QuoterV2 multihop**; optional **`current_price_usd × balance`**. **Zero `FE_USD` with non-zero WETH in MetaMask** usually means **on-chain `balanceOf(WETH)` is zero** for **`WALLET=`** (confirm **Polygonscan** holdings for contract **`0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619`**, or a different token MetaMask labels “WETH”), or a **failed read**—look for **`BALANCE READ FAILED`** in `real_cron.log`. |
