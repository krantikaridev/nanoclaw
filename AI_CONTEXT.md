...
- **Minimum net-edge filter (v1, active — conservative defaults May 2026)** (`modules/swap_executor.py`): **USDC→equity** (X-Signal) and **USDT→WMATIC** (main strategy) entries must show estimated **net** edge ≥ **`MIN_NET_EDGE_PCT`** (env, default **2.5%**; module `_MIN_NET_EDGE_PCT`) after **`MIN_NET_EDGE_FEE_BUFFER_PCT`** (default **0.75%** swap fee/slippage reserve) and rough gas (`effective gross − gas`, as % of notional). **Gross planning**: X-SIGNAL uses `plan_x_signal_gross_edge_pct(signal, upside_pct)` on `TradeDecision.expected_gross_edge_pct` (strength-scaled heuristic, capped upside realization); main BUY uses **`MAIN_STRATEGY_ENTRY_EDGE_FRAC` × `TAKE_PROFIT_PCT`** (default **0.70**) unless **`MAIN_STRATEGY_ENTRY_EDGE_PCT` > 0**. **Fail-closed** when notional is missing/zero. Gate runs **before** dust defer on X-Signal / main BUY / stable-rotation fallback. Planning gas: **`NET_EDGE_PLANNING_GAS_GWEI`** (default **120** gwei, no RPC); execution re-check uses live `get_gas_status()` gas. Exits and copy paths unchanged. Once per process: `[nanoclaw] MIN_NET_EDGE_ACTIVE | floor=…% | fee_buffer=…% | planning_gas=…gwei | directions=…`. Rejection: `[nanoclaw] Low edge rejected | … | expected_net_return_pct=…% | reason=below_min_net_edge (floor=…%) | stage=planning|execution`. Structured: `decision_log` **REJECT** / `below_min_net_edge` for X-Signal and main strategy. Tune **`MIN_NET_EDGE_PCT`** down only with logged fill/PnL evidence.
...
- **X-SIGNAL execution quality** (Signal-driven — May 2026; **CRITICAL for rotation PnL**): USDC→equity BUYs that pass the dynamic effective gate set `gated_enhanced_execution` on the plan and `x_signal_gated_execution` on `TradeDecision`. At swap time, `modules/swap_executor._resolve_x_signal_enhanced_fallback_execution` applies fallback-router slippage **9500 / 12500 bps** (`X_SIGNAL_GATED_TRADE_*` env) plus tiered `min_out` buffer (`X_SIGNAL_GATED_TRADE_MIN_OUT_EXTRA_BPS`, default **100**; +`X_SIGNAL_HIGH_CONVICTION_MIN_OUT_EXTRA_BPS` when `|signal| ≥ 0.90`). Small high-conviction (≤$12, `|signal| ≥ 0.85`) uses `X_SIGNAL_SMALL_HIGH_CONVICTION_*` slippage plus `X_SIGNAL_SMALL_HIGH_CONVICTION_MIN_OUT_EXTRA_BPS`. **`swap_executor.approve_and_swap`** ramps slippage **primary → mid → retry** (`_x_signal_fallback_slippage_ramp`, 3 steps when gap ≥ 400 bps), re-quotes after **`X_SIGNAL_FALLBACK_REQUOTE_DELAY_SECONDS`** (default **1.5s**) before each retry, and picks the **best V3 fee tier** among **500 / 3000 / 10000**. **STF backoff:** each STF revert applies a short per-asset cooldown (`X_SIGNAL_STF_FAILURE_COOLDOWN_SECONDS`, default **600s**); after **`X_SIGNAL_STF_PAUSE_AFTER_FAILURES`** (default **2**) failures, a long pause (`X_SIGNAL_STF_PAUSE_SECONDS`, default **3600s**); state key `x_signal_stf_backoff`. Logs: `[nanoclaw] X-SIGNAL execution quality | EXEC ATTEMPT|SUCCESS|FAILED|STF failure recorded|STF cooldown|pausing…`; router `[nanoclaw] [FALLBACK ROUTER] X-SIGNAL slippage ramp|re-quote delay|min_out buffer applied`; plan `[nanoclaw-av] X-SIGNAL gated trade eligible — enhanced execution on swap`.
...
- **P2 Profit-Take Relief** (TEMPORARY SPRINT FIX — May 2026; revert after sprint window):
  ...
  - **Tiered P2/force (Signal-Driven Rotation — May 2026)**: reduces over-reliance on high WMATIC balance via **`stack_tier`** (`low` / `moderate` / `healthy` in `modules/swap_executor.py`):
    - **Low** (stack **< $7**): P2 wm **$1.75**, signal **0.45**, force wm **$1.75**, force after **2** idle cycles.
    - **Moderate** (**$7–$15**): P2 wm **$4**, signal **0.50**, force wm **$4**, force after **3** cycles (between depleted and healthy).
    - **Healthy** (≥ **$15**): P2 wm **$7**, signal **0.55**, force wm **$5.5**, force after **4** cycles.
  ...
  - **Long-idle fallback (May 2026)**: after **5** / **6** / **8** cycles without a WMATIC→stable exit (low / moderate / healthy tier), **`long_idle_active`** lowers force notional floor to **$1.35** and force wm floor to **$1.50** so micro rotations can still run (gas-safe; not sub-dollar). Log: `[nanoclaw] MAIN_STRATEGY long idle fallback | …`.
  - **Idle WMATIC rotation sell (Signal-Driven Rotation — May 2026)**: when stack is **below** `MAIN_STRATEGY_TP_TRIGGER_WMATIC_USD` (~$52) and idle cycles meet tiered P2/force thresholds (`_main_strategy_idle_rotation_sell_decision`), `select_main_strategy_trade` prefers a **small WMATIC→USDT** slice (**35%** when stack **< $7**, notional floor **$1.35**; else `MAIN_STRATEGY_RESERVE_SELL_FRACTION` / **$1.80** floor) over USDT→WMATIC accumulation. Reasons: `low_wmatic_idle_rotation`, `moderate_wmatic_idle_rotation`, `healthy_wmatic_idle_rotation`.
