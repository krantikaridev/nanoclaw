...
- **P2 Profit-Take Relief** (TEMPORARY SPRINT FIX — May 2026; revert after sprint window):
  ...
  - **Force small profit take** (TEMPORARY SPRINT FIX — capital rotation): `state.profit_take_rotation.cycles_since_exit` increments each `determine_trade_decision`; resets on successful WMATIC→stable exit. After tiered idle cycles without a WMATIC→stable profit take, stack/notional meet force floors (healthy: wm **≥ $5.5**, notional **≥ $5**; long-idle can drop to **$1.35** / **$1.50** wm) and force-allow P2 relief — bypasses standard P2 **$7** wm / **$5.00** notional / **0.55** signal gates and **`min_notional_usd`** like the override path. Log: `[nanoclaw] FORCE small profit take | WMATIC=$X.XX healthy, no exit for N cycles | notional=$Y.YY | bypassing min_notional`; P2 reason `force_no_exit_cycles`.
