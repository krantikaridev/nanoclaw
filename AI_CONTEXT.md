...
- **Minimum net-edge filter (v1)** (`modules/swap_executor.py`): before swap execution, **USDC→equity** (X-Signal) and **USDT→WMATIC** (main strategy) entries must show estimated **net** edge ≥ **`MIN_NET_EDGE_PCT`** (env, default **1.75%**) after a rough gas estimate (`expected gross − gas`, as % of notional). Exits and copy paths are unchanged. Log: `[nanoclaw] Trade rejected | Low edge | expected_net=…%`. First quality gate toward learning from low-edge mistakes.
...
- **X-SIGNAL plan ordering** (`modules/signal.py`): eligible assets sorted by cooldown-ready → conviction tier → **quality score** (`|signal|`, optional `track_record_score` / `upside_pct` in JSON) → `|signal|`; winner prefers force-eligible / high-conviction **BUY** plans. Optional noise filter: `X_SIGNAL_MIN_ACTIONABLE_STRENGTH`, `X_SIGNAL_MIN_UPSIDE_PCT_FOR_WEAK` (0 = off). Structured logs: `X-SIGNAL DECISION | symbol=… | action=ACCEPT|REJECT | reason=…`.
...
- **P2 Profit-Take Relief** (TEMPORARY SPRINT FIX — May 2026; revert after sprint window):
  ...
  - **Force small profit take** (TEMPORARY SPRINT FIX — capital rotation): `state.profit_take_rotation.cycles_since_exit` increments each `determine_trade_decision`; resets on successful WMATIC→stable exit. After **4+** cycles without a WMATIC→stable profit take, stack **≥ $5.5** (lowered from $6.5 for current ~$5.7 WMATIC stacks) and notional **≥ $1.80** (below `MIN_TRADE_USD`) force-allow P2 relief — bypasses standard **$7** / **$2.00** / **0.55** gates and **`min_notional_usd`** like the override path. Log: `[nanoclaw] FORCE small profit take | WMATIC=$X.XX healthy, no exit for N cycles | notional=$Y.YY | bypassing min_notional`; P2 reason `force_no_exit_cycles`.
