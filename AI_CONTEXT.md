...
- **X-SIGNAL execution quality** (Signal-driven — May 2026; **CRITICAL for rotation PnL**): without reliable on-chain fills, signal-driven rotation burns gas and never moves capital. **Every** USDC→equity X-SIGNAL BUY uses enhanced fallback execution via `modules/swap_executor._resolve_x_signal_enhanced_fallback_execution` (tiers: **small** ≤$12 & `|signal|≥0.85` → `X_SIGNAL_SMALL_HIGH_CONVICTION_*`; **gated** when `gated_enhanced_execution` / `x_signal_gated_execution` or notional ≥ dynamic gate → `X_SIGNAL_GATED_TRADE_*` **9500/12500** bps; **default** → `X_SIGNAL_DEFAULT_*` **7000/11000** bps). Tiered `min_out` buffer: gated base **100** bps + high-conviction add-on (`|signal|≥0.90` full `X_SIGNAL_HIGH_CONVICTION_MIN_OUT_EXTRA_BPS`, **≥0.85** half). **`swap_executor.approve_and_swap`** ramps slippage in up to **four** steps when primary→retry gap ≥ **2000** bps (`_x_signal_fallback_slippage_ramp`), re-quotes after **`X_SIGNAL_FALLBACK_REQUOTE_DELAY_SECONDS`** (default **1.5s**), and quotes V3 fees **500/3000/10000** with **stable preference** for the **3000** pool when within **`X_SIGNAL_STABLE_FEE_PREFER_BPS`** (default **75**) of the best quote. **STF backoff:** each STF revert → short per-asset cooldown (`X_SIGNAL_STF_FAILURE_COOLDOWN_SECONDS`, default **600s**); after **`X_SIGNAL_STF_PAUSE_AFTER_FAILURES`** (default **2**) → long pause (`X_SIGNAL_STF_PAUSE_SECONDS`, default **3600s**); state `x_signal_stf_backoff`; STF-paused symbols sort **last** in `modules/signal._order_eligible_x_signal_candidates` and plan pick. Logs: `[nanoclaw] X-SIGNAL execution quality | EXEC ATTEMPT|SUCCESS|FAILED` (includes `slippage_bps`, `min_out`, `fee_tier`, `revert`); router `[FALLBACK ROUTER] X-SIGNAL slippage ramp|re-quote delay|min_out buffer`; plan `[nanoclaw-av] X-SIGNAL gated trade eligible — enhanced execution on swap`.
