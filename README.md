...
`scripts/pnl_report.py` now prioritizes **live on-chain snapshots** (`WALLET BALANCE` + `Real USDT` pairing and direct `Real USDT` lines) ahead of `MANUAL CORRECT BALANCE` lines, then chooses the most recent usable live snapshot. Manual snapshots are used as fallback when no usable live snapshot is found.
