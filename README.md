...
`scripts/pnl_report.py` now prioritizes **live on-chain snapshots** (`WALLET BALANCE` + `Real USDT` pairing, then `Real USDT`) ahead of `MANUAL CORRECT BALANCE` lines. Manual snapshots are used as fallback when no usable live snapshot is found.
