...
| `nanodaily` | Daily health snapshot: **LOOKBACK** table (current log total vs `portfolio_history.csv` at 1h…~1m horizons), baseline/session PnL, bypass/cooldown/**trade-skip** counters (lifetime + 24h UTC + top reasons), commit, TEST_MODE |
| `nano_watch` | **`bash scripts/nano_watch.sh`**: append-only **12h** monitoring loop (default 30m interval) to `~/nanoclaw_watch.log` — RPC, PnL grep lines, last `Risk=` / `EXEC` / `FE STABLE RUNWAY` from `real_cron.log`; see **`docs/readme-vm-update.md`** § Monitoring |
...
Session baseline can be reset manually with `nanopnl --reset-session` (or `nanostatus --reset-session` / `nanorestart --reset-session`). See **`docs/OPERATOR_PNL_MARK_VS_TRADE.md`** when Session PnL moves without trades (FE spot cache / MetaMask Polygon parity).
...
**FE stable runway (Cleanup #6)**: when stables are critically low but the book is FE-heavy and WMATIC cannot rebuild the buffer, the bot trims the largest followed-equity holding toward `FE_STABLE_RUNWAY_TARGET_STABLE_USD` and blocks `USDC→EQUITY` BUY until stables recover. Knobs: `FE_STABLE_RUNWAY_ENABLED`, `FE_STABLE_RUNWAY_MIN_FE_SHARE`, `FE_STABLE_RUNWAY_TARGET_STABLE_USD` (see `.env.example`). Operator grep: `grep -E 'FE STABLE RUNWAY' real_cron.log | tail -20`.
...
- **`control.json`** is **gitignored** operator runtime state. **`nanoup`** snapshots it to **`.runtime/control.json.bak`** before git and restores after (even on errors). It is not part of env sync. To **unpause and keep it** while the external layer runs, set `"paused": false` and **`"operator_pause_lock": true`** (see **`control.json.example`**).
