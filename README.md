...
`scripts/pnl_report.py` now prioritizes **live on-chain snapshots** (`WALLET BALANCE` + `Real USDT` pairing and direct `Real USDT` lines) ahead of `MANUAL CORRECT BALANCE` lines, then chooses the most recent usable live snapshot. Snapshot validation now rejects non-finite (`NaN`/`inf`), negative, or clearly unreasonable component values before fallback selection. Manual snapshots are used as fallback when no usable live snapshot is found.

**USDC top-up for X-Signal equity** is handled inside the bot (`try_x_signal_equity_decision`), not as a separate shell alias. Configure `X_SIGNAL_USDC_SAFE_FLOOR`, `X_SIGNAL_AUTO_USDC_TARGET`, `X_SIGNAL_AUTO_USDC_TOPUP_ENABLED`, and `X_SIGNAL_AUTO_USDC_MIN_SWAP_USD` in `.env` (see `.env.example`). Top-up prefers `USDT -> USDC` and falls back to `WMATIC -> USDC`. On-chain USDC reads now retry before fallback; tune with `X_SIGNAL_ONCHAIN_USDC_RETRY_ATTEMPTS`. Top-up evaluation now logs a clear `AUTO-USDC consider` line before guard checks and only executes when at least one BUY asset is cooldown-ready.
