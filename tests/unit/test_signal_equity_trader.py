...
def test_x_signal_gated_buy_logs_enhanced_execution_eligibility(monkeypatch, capsys):
    """TEMPORARY: BUYs passing $18 gate log enhanced execution eligibility."""
    s = _build_strategy_tuned(min_trade_usdc=4.0, max_trade_usdc=200.0)
    monkeypatch.setattr(strategy_module, "_HARD_BYPASS_MIN_TRADE_USD", 1.0)
    monkeypatch.setattr(
        SignalEquityTrader,
    ...
    assert plan is not None
    assert reason is None
    assert "X-SIGNAL gated trade eligible — enhanced execution on swap" in capsys.readouterr().out
