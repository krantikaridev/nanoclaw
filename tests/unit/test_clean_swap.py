...
def _reset_auto_usdc_failure_state() -> None:
    from modules import signal as signal_module

    signal_module._AUTO_USDC_FAILURE_STATE["next_retry_ts"] = 0.0
    signal_module._AUTO_USDC_FAILURE_STATE["last_failure_usdc"] = -1.0
...
def test_determine_trade_decision_profit_take_balance_relief_skipped_when_wm_stack_small(monkeypatch, capsys):
    ...
    assert any("profit_take_dust_deferred" in reason for reason in skipped)
    assert "PROFIT_TAKE DUST DEFER" in captured
    assert "Main strategy small profit take allowed (P2 relief)" not in captured
...
def test_fe_stable_runway_tiered_allows_high_signal_buy_below_runway_target(monkeypatch, capsys):
    """Tiered runway: stables $27 / 79% FE — allow capped BUY when signal ≥ 0.85."""
    from modules import signal as signal_module
    ...
    monkeypatch.setattr(signal_module.cfg, "PROTECTION_FLUCTUATION_USDT_THRESHOLD", 12.0, raising=False)
    monkeypatch.setattr(signal_module.cfg, "ALLOW_REDUCED_HIGH_RISK_XSIGNAL", True, raising=False)
    monkeypatch.setattr(signal_module.cfg, "FE_STABLE_RUNWAY_TIERED_ENABLED", True, raising=False)
    monkeypatch.setattr(signal_module.cfg, "FE_STABLE_RUNWAY_TIERED_MIN_SIGNAL", 0.85, raising=False)
    monkeypatch.setattr(signal_module.cfg, "FE_STABLE_RUNWAY_TIERED_MAX_NOTIONAL_USD", 10.0, raising=False)
    monkeypatch.setattr(signal_module.cfg, "STAGE_SEED_USD", 132.0, raising=False)
    monkeypatch.setattr(signal_module.cfg, "OPERATING_RESERVE_ENABLED", True, raising=False)
    monkeypatch.setattr(signal_module.cfg, "OPERATING_RESERVE_PCT", 10.0, raising=False)
    ...
    captured = capsys.readouterr().out
    assert swap_exec._FE_STABLE_RUNWAY_TIERED_ALLOW_LOG in captured
    assert swap_exec._FE_STABLE_RUNWAY_DEFER_BUY_LOG not in captured
...
def test_main_low_stables_dust_rebuild_min_trade_guard_bypassed(monkeypatch, capsys):
    """Planning-approved low-stables dust rebuild clears MIN_TRADE_USD at execution."""
    monkeypatch.setenv("POLYGON_PRIVATE_KEY", "0x" + "a" * 64)
    state = {"low_stables_dust_rebuild": {"pending_execution": True}}
    monkeypatch.setattr(clean_swap, "load_state", lambda: state)
    monkeypatch.setattr(
        clean_swap,
        "get_balances",
        lambda: clean_swap.Balances(usdt=5.0, wmatic=80.0, pol=1.0, usdc=4.0),
    )
    monkeypatch.setattr(clean_swap, "has_active_lock", lambda: False)
    monkeypatch.setattr(clean_swap, "create_lock", lambda: None)
    monkeypatch.setattr(clean_swap, "release_lock", lambda: None)
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: clean_swap.TradeDecision(
            direction="WMATIC_TO_USDC",
            amount_in=int((notional / wmatic_price) * 1_000_000_000_000_000_000),
            trade_size=notional,
            message="dust rebuild",
        ),
    )
    monkeypatch.setattr(
        swap_exec,
    ...
    monkeypatch.setattr(swap_exec, "_profit_take_balance_relief_bypass_allowed", lambda *_a, **_k: False)
    monkeypatch.setattr(swap_exec, "_main_strategy_mild_loss_rotation_min_notional_bypass_allowed", lambda *_a, **_k: False)
...
def test_try_x_signal_equity_decision_skips_buys_when_risk_high(monkeypatch, capsys):
    from modules import signal as signal_module
    ...
    decision = clean_swap.try_x_signal_equity_decision(
        clean_swap.Balances(usdt=10.0, wmatic=10.0, pol=1.0, usdc=20.0),
        dry_run=True,
    )
    out = capsys.readouterr().out
...
def test_try_x_signal_equity_decision_pauses_buys_when_risk_medium(monkeypatch, capsys):
    ...
    assert decision is None
    assert "Risk: MEDIUM → Pausing WMATIC BUYs this cycle" in out


def test_try_x_signal_equity_decision_allows_weth_buy_when_risk_medium(monkeypatch, capsys):
    from modules import signal as signal_module
    ...
    monkeypatch.setattr(clean_swap, "ENABLE_X_SIGNAL_EQUITY", True)
    monkeypatch.setattr(
        clean_swap, "_load_followed_equities_json_dict", lambda: {"enabled": True, "min_signal_strength": 0.60}
    )
    monkeypatch.setattr(clean_swap, "_effective_equity_signal_min", lambda cfg: 0.60)
    monkeypatch.setattr(clean_swap, "X_SIGNAL_EQUITY_TRADER", _BaseTrader())
    monkeypatch.setattr(clean_swap, "_tuned_signal_equity_trader", lambda min_strength: _TunedTrader())
    ...
    assert decision is not None
    assert decision.direction == "USDC_TO_EQUITY"
    assert "X-SIGNAL BUY SKIPPED | symbol=WETH_ALPHA" not in out
...
def test_try_x_signal_high_conviction_bypasses_high_gas_and_forces_usdc_topup(monkeypatch, capsys):
    ...
    assert "short-circuit cycle after AUTO-USDC failure" in out
...
def test_try_x_signal_short_circuits_cycle_after_failed_auto_usdc(monkeypatch):
    from modules import signal as signal_module
    ...
    monkeypatch.setattr(clean_swap, "_effective_equity_signal_min", lambda cfg: 0.60)
    monkeypatch.setattr(clean_swap, "X_SIGNAL_EQUITY_TRADER", _BaseTrader())
    monkeypatch.setattr(clean_swap, "_tuned_signal_equity_trader", lambda min_strength: tuned)
    monkeypatch.setattr(clean_swap, "can_trade_asset", lambda symbol, now=None, cooldown_seconds=0: True)
    monkeypatch.setattr(clean_swap, "get_token_balance", lambda *_args, **_kwargs: 0.0)
    monkeypatch.setattr(clean_swap, "ensure_usdc_for_x_signal", lambda **_kwargs: False)
    monkeypatch.setattr(
        clean_swap,
        "get_balances",
        lambda: clean_swap.Balances(usdt=40.0, wmatic=10.0, pol=1.0, usdc=2.0),
    ...
    assert decision is None
    assert tuned.build_plan_calls == 0


def test_try_x_signal_auto_usdc_failure_backoff_skips_immediate_retry(monkeypatch, capsys):
    from modules import signal as signal_module
    ...
    monkeypatch.setattr(clean_swap, "ENABLE_X_SIGNAL_EQUITY", True)
    monkeypatch.setattr(clean_swap, "X_SIGNAL_AUTO_USDC_TOPUP_ENABLED", True)
    monkeypatch.setattr(signal_module, "X_SIGNAL_AUTO_USDC_FAIL_COOLDOWN_SECONDS", 300)
    monkeypatch.setattr(clean_swap, "_load_followed_equities_json_dict", lambda: {"enabled": True, "min_signal_strength": 0.60})
    monkeypatch.setattr(clean_swap, "_effective_equity_signal_min", lambda cfg: 0.60)
    monkeypatch.setattr(clean_swap, "X_SIGNAL_EQUITY_TRADER", _BaseTrader())
    monkeypatch.setattr(clean_swap, "_tuned_signal_equity_trader", lambda min_strength: _TunedTrader())
    ...
    out = capsys.readouterr().out
    assert calls["count"] == 1
    assert "AUTO-USDC skipped — failure backoff active" in out
