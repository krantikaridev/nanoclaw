...
def test_recovery_pass_does_not_increment_failure_streak(monkeypatch):
    _reset_rpc_state()
    ...
    monkeypatch.setattr(nc_cfg, "_RPC_RECOVERY_PASS_DELAY_SEC", 0.0)

    try:
        nc_cfg.connect_web3(urls=[ep])
    except RuntimeError:
        pass

    assert int(nc_cfg._RPC_ENDPOINT_FAILURE_STREAK.get(ep, 0)) == 1
    assert nc_cfg.rpc_endpoint_in_cooldown(ep) is False
...
def test_connect_web3_recovery_pass_after_cooldown(monkeypatch):
    ...
    client = nc_cfg.connect_web3(urls=[ep])
