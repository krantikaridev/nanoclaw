...
- **`nanoup` merges template → `.env` but preserves RPC keys, secrets, and **`WALLET=`**—not `MAX_GWEI`** (see `ENV_APPLY_PRESERVE_KEYS` in `nanoclaw/env_sync.py`). **`MIN_POL_FOR_GAS`** always comes from `.env.example` on nanoup (`ENV_APPLY_FORCE_TEMPLATE_KEYS`). After deploy, verify **`grep '^WALLET=' .env`** still matches your custody address.
