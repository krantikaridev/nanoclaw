...
- Strategy precedence note:
  ...
  - **TEMPORARY (48-hour sprint)**: `PROFIT_TAKE` may still run small WMATIC→USDT/USDC notionals (≥ $3.50 and below `MIN_TRADE_USD`) when total WMATIC USD equivalent exceeds ~$7 and derived signal strength ≥ 0.55 (`modules/swap_executor._profit_take_balance_relief_bypass_allowed`); look for `[nanoclaw] P2 relief check` and `[nanoclaw] Main strategy small profit take allowed (P2 relief)` in logs.
