...
# Block tiered USDC→EQUITY for N hours after tiered fill or WMATIC→stable rebuild (stables recovery).
FE_STABLE_RUNWAY_TIERED_COOLDOWN_ENABLED = env_bool("FE_STABLE_RUNWAY_TIERED_COOLDOWN_ENABLED", True)
FE_STABLE_RUNWAY_TIERED_COOLDOWN_HOURS = env_float("FE_STABLE_RUNWAY_TIERED_COOLDOWN_HOURS", 4.0)
FE_STABLE_RUNWAY_TIERED_COOLDOWN_AFTER_REBUILD = env_bool(
...
# Dynamic FE-share bands for de-risk max trim (nanoclaw/fe_dynamic_trim.py).
FE_STABLE_RUNWAY_DERISK_DYNAMIC_ENABLED = env_bool("FE_STABLE_RUNWAY_DERISK_DYNAMIC_ENABLED", False)
FE_STABLE_RUNWAY_DERISK_HIGH_FE_SHARE = env_float("FE_STABLE_RUNWAY_DERISK_HIGH_FE_SHARE", 0.85)
FE_STABLE_RUNWAY_DERISK_HIGH_FE_MAX_TRIM_USD = env_float(
...
DRAWDOWN_THROTTLE_TRIGGER_PCT = env_float("DRAWDOWN_THROTTLE_TRIGGER_PCT", -1.0)
DRAWDOWN_THROTTLE_NOTIONAL_MULT = env_float("DRAWDOWN_THROTTLE_NOTIONAL_MULT", 0.5)
# V4-play: dual-window auto_unpause requires 8h AND 12h window PASS (external_layer/dual_window_unpause.py).
EXTERNAL_AUTO_UNPAUSE_REQUIRE_DUAL_WINDOW = env_bool("EXTERNAL_AUTO_UNPAUSE_REQUIRE_DUAL_WINDOW", True)
EXTERNAL_AUTO_UNPAUSE_SHORT_HOURS = env_float("EXTERNAL_AUTO_UNPAUSE_SHORT_HOURS", 8.0)
# V4-play: max USDC_TO_EQUITY entry fills per UTC day on small books (nanoclaw/play_budget.py).
PLAY_BUDGET_ENABLED = env_bool("PLAY_BUDGET_ENABLED", False)
PLAY_BUDGET_MAX_FILLS_PER_UTC_DAY = env_int("PLAY_BUDGET_MAX_FILLS_PER_UTC_DAY", 2)
PLAY_BUDGET_TOTAL_USD_CEILING = env_float("PLAY_BUDGET_TOTAL_USD_CEILING", 200.0)
# V4-play: cap X_SIGNAL max trade when 12h window < 0% (nanoclaw/negative_window_x_signal_cap.py).
X_SIGNAL_NEGATIVE_WINDOW_CAP_ENABLED = env_bool("X_SIGNAL_NEGATIVE_WINDOW_CAP_ENABLED", False)
X_SIGNAL_NEGATIVE_WINDOW_CAP_HOURS = env_float("X_SIGNAL_NEGATIVE_WINDOW_CAP_HOURS", 12.0)
X_SIGNAL_NEGATIVE_WINDOW_MAX_TRADE_USD = env_float("X_SIGNAL_NEGATIVE_WINDOW_MAX_TRADE_USD", 10.0)
# Adverse churn guard: log + runtime flag when red window + high fills (nanoclaw/adverse_churn_guard.py).
ADVERSE_CHURN_GUARD_ENABLED = env_bool("ADVERSE_CHURN_GUARD_ENABLED", False)
ADVERSE_CHURN_GUARD_FILL_MULT = env_float("ADVERSE_CHURN_GUARD_FILL_MULT", 0.5)
# High-stable WMATIC rotation: capped USDC→WMATIC when stables ample (nanoclaw/high_stable_wmatic_rotation.py).
MAIN_STRATEGY_HIGH_STABLE_WMATIC_ROTATION_ENABLED = env_bool(
    "MAIN_STRATEGY_HIGH_STABLE_WMATIC_ROTATION_ENABLED", False
)
MAIN_STRATEGY_HIGH_STABLE_WMATIC_MIN_STABLE_USD = env_float(
    "MAIN_STRATEGY_HIGH_STABLE_WMATIC_MIN_STABLE_USD", 30.0
)
MAIN_STRATEGY_HIGH_STABLE_WMATIC_MAX_NOTIONAL_USD = env_float(
    "MAIN_STRATEGY_HIGH_STABLE_WMATIC_MAX_NOTIONAL_USD", 10.0
)
MAIN_STRATEGY_HIGH_STABLE_WMATIC_MIN_SIGNAL = env_float(
    "MAIN_STRATEGY_HIGH_STABLE_WMATIC_MIN_SIGNAL", 0.85
)
MAIN_STRATEGY_HIGH_STABLE_WMATIC_MAX_FE_SHARE = env_float(
...
# Auto-unpause: require N consecutive external-layer ticks with window >= floor + buffer (default off).
EXTERNAL_AUTO_UNPAUSE_HYSTERESIS_ENABLED = env_bool(
    "EXTERNAL_AUTO_UNPAUSE_HYSTERESIS_ENABLED", False
)
EXTERNAL_AUTO_UNPAUSE_HYSTERESIS_TICKS = env_int("EXTERNAL_AUTO_UNPAUSE_HYSTERESIS_TICKS", 6)
EXTERNAL_AUTO_UNPAUSE_WINDOW_BUFFER_PCT = env_float(
