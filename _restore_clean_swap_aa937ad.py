...
def _sorted_and_eligible_equities(
    ...
    for a in assets_list:
        ...
        if is_eligible:
            ...
            print(
                f"{_nanolog()}  Γ£à ELIGIBLE | {a.symbol:12s} | signal={strength:6.3f} | "
                f"by: {reason}"
            )
        else:
            print(
                f"{_nanolog()}  Γ¥î FILTERED | {a.symbol:12s} | signal={strength:6.3f} | "
...
def mark_wallet_traded(
    ...
    print(f"≡ƒôî Wallet {wallet_address[:8]}... cooldown started ({cooldown_seconds}s)")
...
def mark_asset_traded(symbol: str, now: Optional[float] = None, cooldown_seconds: int = 0) -> None:
    ...
    if cooldown_seconds:
        print(f"≡ƒôî Asset {symbol} cooldown started ({int(cooldown_seconds)}s)")
...
def ensure_pol_for_trade(min_pol: float = 0.025) -> bool:
    ...
    if not key:
        print(f"{_nanolog()}AUTO-POL skipped ΓÇö no private key")
        return False

    print(f"≡ƒöä AUTO-POL | Topping up 0.03 POL (current: {current_pol})")
    ...
    if balances.wmatic < desired_topup_pol and usdt_swap_amount >= 6.0:
        ...
        if not ok:
            print(f"{_nanolog()}AUTO-POL warn ΓÇö USDTΓåÆWMATIC leg failed, trying WMATIC unwrap fallback")
        balances = get_balances()

    unwrap_pol = min(desired_topup_pol, float(balances.wmatic) * 0.95)
    if unwrap_pol <= 0:
        print(f"{_nanolog()}AUTO-POL skipped ΓÇö insufficient WMATIC/USDT for top-up")
        return False
    ...
    try:
        ...
        if int(receipt.get("status", 0)) != 1:
            print(f"{_nanolog()}AUTO-POL failed ΓÇö WMATIC unwrap reverted")
            return False
        final_pol = float(get_pol_balance())
        if final_pol >= float(min_pol):
            print("Γ£à AUTO-POL | Top-up successful")
            return True
        print(
            f"{_nanolog()}AUTO-POL failed ΓÇö POL still low "
            f"(polΓëê{final_pol:.4f}, needΓëÑ{float(min_pol):.4f})"
        )
        return False
    except Exception as e:
        print(f"{_nanolog()}AUTO-POL failed ΓÇö {e}")
        return False
...
def build_protection_exit_decision(
    ...
    if reason == "PER_TRADE_EXIT" and open_trade:
        ...
        message = (
            f"≡ƒ¢í∩╕Å PROTECTION EXIT: {'strong TP hit' if gain_pct >= strong_signal_tp else 'TP hit'} | "
            f"buy ${buy_price:.4f} -> now ${current_price:.4f} "
            f"({gain_pct:.2f}%) | selling {sell_fraction * 100:.0f}% WMATIC"
        )
    else:
        message = f"≡ƒ¢í∩╕Å PROTECTION TRIGGERED: {reason} ΓÇö Force selling"
...
def build_profit_exit_decision(profit_signal: dict, wmatic_balance: float) -> TradeDecision:
    ...
    return TradeDecision(
        ...
        amount_in=int(wmatic_balance * sell_fraction * 1e18),
        message=(
            f"≡ƒÆ░ EXIT SIGNAL: {profit_signal['reason']} | {profit_signal['message']} | "
...
def select_copy_trade(balances: Balances, wallets: list[str]) -> TradeDecision:
    ...
    return TradeDecision(
        ...
        message="≡ƒöä REAL POLYCOPY MODE (28%) - Monitoring live wallets",
    )

def _strong_x_signal_buy_present() -> bool:
    """True if followed_equities has a BUY with strength ΓëÑ X_SIGNAL_STRONG_THRESHOLD (same bar as equity trades)."""
...
def ensure_usdc_for_x_signal(min_usdc: float = 8.0, min_wmatic_value: float = 15.0, force: bool = False) -> bool:
    """If USDC is below ``min_usdc`` and (a strong X-Signal BUY is active or force=True), swap WMATICΓåÆUSDC (preferred) or USDTΓåÆUSDC."""
    balances = get_balances()
    ...
    if not gst.get("ok", False):
        print(f"{_nanolog()}AUTO-USDC skipped ΓÇö gas/POL guard")
        return False

    key = os.getenv("POLYGON_PRIVATE_KEY") or os.getenv("PRIVATE_KEY")
    if not key:
        print(f"{_nanolog()}AUTO-USDC skipped ΓÇö no private key")
        return False
    ...
    try:
        ...
        wmatic_price = 0.0
    if wmatic_price <= 0:
        print(f"{_nanolog()}AUTO-USDC skipped ΓÇö WMATIC price unavailable")
        return False
    ...
    if use_wmatic_first:
        ...
        if amount_wei > 0:
            print(f"≡ƒöä AUTO-USDC | Swapping ${swap_usd:.2f} from WMATIC ΓåÆ USDC (gas optimized)")
            ok = _run(_swap_wmatic(amount_wei))
            if ok:
                print(f"≡ƒöä AUTO-USDC | Swapped ${swap_usd:.2f} from WMATIC ΓåÆ USDC (gas optimized)")
            if ok and get_balances().usdc >= min_usdc:
                return True
        after = get_balances()
        if after.usdt >= 0.5:
            print("≡ƒöä AUTO-USDC | Using USDT path (fallback)")
...
def try_x_signal_equity_decision(balances: Balances, *, dry_run: bool = False) -> Optional[TradeDecision]:
    """Highest-priority iteration: eligible assets ├ù SignalEquityTrader.build_plan; first plan wins."""
    ...
    if not dry_run:
        ...
        if probe.usdc < X_SIGNAL_USDC_SAFE_FLOOR:
            print(
                f"≡ƒÜÇ PROACTIVE USDC TOP-UP triggered (have ${probe.usdc:.2f} < ${X_SIGNAL_USDC_SAFE_FLOOR:.2f}, "
    ...
    if high_conviction and force_high_conviction:
        print(
            f"≡ƒÜÇ HIGH-CONVICTION MODE ACTIVE ΓÇö {high_conviction_threshold:.2f}+ signal present, "
            "force_high_conviction enabled, gas protection relaxed for this cycle"
        )
    elif high_conviction:
        print(
            f"ΓÜá∩╕Å HIGH-CONVICTION signal present ({high_conviction_threshold:.2f}+), but force_high_conviction is disabled"
        )

    print(f"≡ƒƒª X-SIGNAL EQUITY CHECK | Checking {len(assets)} assets")
    if not cfg_enabled:
        result = f"NO TRADE | followed_equities disabled (enabled=false in {FOLLOWED_EQUITIES_PATH})"
        print(f"≡ƒƒª X-SIGNAL EQUITY | {result}")
    elif not assets:
        ...
        print(
            f"≡ƒƒª X-SIGNAL EQUITY | No valid plan this cycle "
        ...
        if high_conviction:
            print(
                f"≡ƒÜÇ HIGH-CONVICTION GAS OVERRIDE | {strong_thr:.2f}+ strength detected ΓÇö bypassing 450 gwei limit "
            ...
            if dry_run:
                ...
                if not topup_ok and balances.usdc < X_SIGNAL_HIGH_CONVICTION_PREP_MIN_USDC:
                    print(
                        "≡ƒƒª X-SIGNAL EQUITY | Auto-USDC failed during HIGH-CONVICTION prep "
        ...
        if has_strong_buy and balances.usdc < max(force_floor, auto_usdc_target):
            if dry_run:
                ...
                if not topup_ok and balances.usdc < max(force_floor, min_trade_usdc):
                    print(
                        "≡ƒƒª X-SIGNAL EQUITY | Auto-USDC attempt failed "
                        "(guard/tx) ΓÇö BUY paths may be skipped"
                    )
                    _log_trade_skipped(
            ...
            if balances.usdc >= max(force_floor, min_trade_usdc):
                if dry_run:
                    print(
                        "≡ƒƒª X-SIGNAL EQUITY | DRY-RUN: projected post auto-USDC | Proceeding with BUY analysis"
                    )
                else:
                    print("≡ƒƒª X-SIGNAL EQUITY | USDC ensured via WMATIC/USDT | Proceeding with BUY")
            else:
                print(
                    f"≡ƒƒª X-SIGNAL EQUITY | Auto-USDC insufficient for BUY equity "
                    f"(USDC ${balances.usdc:.2f} < ${max(force_floor, min_trade_usdc):.2f}) ΓÇö BUY paths skipped"
                )
                _log_trade_skipped(
        ...
        if not dry_run and has_strong_buy:
            ...
            if float(balances.pol) < float(MIN_POL_FOR_GAS):
                if AUTO_TOPUP_POL:
                    if ensure_pol_for_trade(min_pol=float(MIN_POL_FOR_GAS)):
                        ...
                        if float(balances.pol) >= float(MIN_POL_FOR_GAS):
                            ...
                            print(
                                ...
                                f"(polΓëê{float(balances.pol):.4f} < {MIN_POL_FOR_GAS:.4f}) ΓÇö BUY paths skipped"
                            )
                        ...
                        print(
                            ...
                            f"(pol<{MIN_POL_FOR_GAS:.4f}) ΓÇö BUY paths skipped"
                        )
                        buy_paths_allowed = False
                        buy_paths_block_reason = (
                    ...
                    print(
                        f"{_nanolog()}POL low (polΓëê{float(balances.pol):.4f} < {MIN_POL_FOR_GAS:.4f}) "
                        f"and AUTO_TOPUP_POL=false ΓÇö BUY paths skipped"
                    )
...
async def evaluate_usdc_copy_trade(
    ...
    *,
    ...
    if not ENABLE_USDC_COPY:
        return TradeDecision(message="Γä╣∩╕Å USDC copy disabled")
    ...
    if not plan:
        return TradeDecision(message="Γä╣∩╕Å No USDC-copy trade this cycle")
...
def select_main_strategy_trade(
    ...
    if balances.usdt < 25:
        return TradeDecision(
            ...
            message=f"≡ƒöä USDT RESERVE PROTECTION: ${balances.usdt:.2f} < $25",
        )
    ...
    if wmatic_value_usd > 52:
        return TradeDecision(
            direction="WMATIC_TO_USDT",
            amount_in=int(balances.wmatic * 0.45 * 1e18),
            message=f"≡ƒöä Taking profit (WMATIC high: ${wmatic_value_usd:.2f})",
        )
    ...
    if wmatic_value_usd < 40 and balances.wmatic > 50:
        return TradeDecision(
            ...
            message=f"≡ƒöä Cutting loss (WMATIC down: ${wmatic_value_usd:.2f})",
        )
    ...
    return TradeDecision(
        ...
        amount_in=int(trade_size * 1_000_000),
        trade_size=trade_size,
        message=f"≡ƒöä Buying WMATIC (hold preferred) | Size: ${trade_size:.2f}",
    )
...
def determine_trade_decision(
    ...
    *,
    ...
    print(
        f"≡ƒöì BALANCES | USDT=${balances.usdt:.2f} USDC=${balances.usdc:.2f} "
        f"WMATICΓëê{(balances.wmatic * current_price):.2f} USD | POL={balances.pol:.4f} "
        f"| copy_targets={len(target_wallets_prelude)}"
    )
    print(
        "≡ƒöì DECISION PATH | precedence: "
        "PROTECTION ΓåÆ PROFIT_TAKE ΓåÆ X_SIGNAL_EQUITY ΓåÆ USDC_COPY | POLYCOPY ΓåÆ MAIN_STRATEGY"
    )
    print(
    ...
    if COPY_TRADE_PCT >= 0.20:
        print(f"≡ƒöÑ AGGRESSIVE MODE ACTIVE | COPY_TRADE_PCT={COPY_TRADE_PCT:.2f}")

    should_force_sell, reason = check_exit_conditions()
    if should_force_sell:
        print(f"≡ƒöì DECISION PATH: PROTECTION ({reason})")
    ...
    if is_copy_trading_enabled() and target_wallets:
        if ENABLE_USDC_COPY:
            ...
            if plan:  # try all eligible assets
                print(f"≡ƒöì DECISION PATH: USDC_COPY | Size ~${plan.trade_size:.2f}")
                print(f"≡ƒƒª USDC COPY ACTIVE | Size: ${plan.trade_size:.2f}")
                cw_usdc = (
                    str(plan.wallet).strip(),
            ...
            print("≡ƒƒª USDC COPY ACTIVE | No eligible copy-trade this cycle")
        else:
            print("≡ƒöì DECISION PATH: POLYCOPY_TARGET_WALLETS")
            return select_copy_trade(balances, target_wallets)

    print(f"≡ƒöì DECISION PATH: MAIN_STRATEGY (WMATICΓëê${current_price:.4f})")
...
async def main(*, dry_run: bool = False) -> None:
    ...
    if has_active_lock():
        print("Γ¢ö Lock active ΓÇö skipping")
        return
    ...
    try:
        ...
        if is_global_cooldown_active(state):
            ...
            print(
                f"{_nanolog()}skip cycle ΓÇö global cooldown ~{remain_s:.0f}s left "
        ...
        if not decision.should_execute:
            ...
            print("Γä╣∩╕Å No actionable trade this cycle")
            return

        if dry_run:
            print(f"≡ƒº¬ DRY RUN: would execute {decision.direction} for amount_in={decision.amount_in}")
            return
        ...
        if pol_now < float(MIN_POL_FOR_GAS):
            if AUTO_TOPUP_POL:
                ...
                if not topup_ok:
                    ...
                    print(f"{_nanolog()}AUTO-POL failed ΓÇö trade blocked (pol<{MIN_POL_FOR_GAS:.3f})")
                    return
            else:
                _log_trade_skipped(f"POL low (have {pol_now:.4f}, need {MIN_POL_FOR_GAS:.4f})")
                print(
                    f"{_nanolog()}POL low (polΓëê{pol_now:.4f} < {MIN_POL_FOR_GAS:.4f}) and AUTO_TOPUP_POL=false ΓÇö trade blocked"
                )
                return
        ...
        if not gas_status["ok"]:
            ...
            if gas_gwei <= 400.0:
                print("ΓÜá∩╕Å High gas but forcing trade (urgent mode)")
            else:
                _log_trade_skipped(
                    f"protection (gas high: {gas_status['gas_gwei']:.2f} gwei > {gas_status['max_gwei']:.2f})"
                )
                print(
                    "Γ¢ö Gas protection active "
        ...
        if tx_hash:
            print("Γ£à Swap executed successfully!")
            ...
            print(f"{_nanolog()}Swap failed ΓÇö per-asset/per-wallet cooldown not applied")

        state["last_run"] = time.time()
        save_state(state)
        print(f"Γ£à Cycle done ΓÇö next in ~{COOLDOWN_MINUTES} min")
    finally:
        release_lock()
