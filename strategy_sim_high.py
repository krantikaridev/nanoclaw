#!/usr/bin/env python3
"""
Nanoclaw v2 - SIM High Confidence Strategy (Madan Pune)
Only trades when SIM shows very strong momentum
"""
...
async def run_real_trade():
    ...
    try:
        with open("skills/autonomous-revenue-engine/capital.json") as f:
            ...
            sim_positive = sim_capital > SIM_CONFIDENCE_THRESHOLD * 1.2  # Stronger threshold for this strategy
        print(f"✅ SIM High Confidence: {'Very Strong' if sim_positive else 'Not enough'} (~{sim_capital/1000:.0f}k)")
    except:
        sim_positive = False

    if not sim_positive:
        print("⏸️ SIM not strong enough for high-confidence strategy - skipping")
        return

    print(f"""
══════════════════════════════════════
🚀 v2 SIM HIGH CONFIDENCE TRADE
...
""")
    ...
    if tx_hash:
        ...
        print(f"✅ SIM High Confidence trade completed. Tx: {tx_hash}")
    else:
        print("❌ SIM High Confidence trade failed")
