...
from constants import ROUTER, ROUTER_ABI, WMATIC, USDT, WALLET, GET_AMOUNTS_OUT_ABI

load_dotenv()

def get_wmatic_price():
...
def log_portfolio(usdt, wmatic, total):
    entry = {"timestamp": datetime.now().isoformat(), "usdt": round(usdt, 2), "wmatic": round(wmatic, 2), "total_value": round(total, 2)}
    if os.path.exists("portfolio_history.json"):
        with open("portfolio_history.json") as f: history = json.load(f)
    else: history = []
    history.append(entry)
    with open("portfolio_history.json", "w") as f: json.dump(history, f, indent=2)

def calculate_pnl():
    with open("trade_history.json") as f: trades = json.load(f)
    ...
    print(f"Current WMATIC        : {wmatic_now:.2f} (${wmatic_value:.2f})  [LIVE: ${wmatic_price:.4f}]")
